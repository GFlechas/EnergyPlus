...
\item
  Zone,Average,HAMT Surface Outside Face Vapor Pressure {[}Pa{]}
\item
  Zone,Average,HAMT Surface Linearization Iterations {[}{]}
\end{itemize}
...
\paragraph{HAMT Surface Outside Face Vapor Pressure {[}Pa{]}}\label{hamt-surface-outside-face-vapor-pressure-pa}

This output is the vapor pressure {[}Pa{]} on the external ``surface'' of the surface. It is the exterior counterpart to the HAMT Surface Inside Face Vapor Pressure output and is useful for interstitial condensation and Glaser-type analyses.

\paragraph{HAMT Surface Linearization Iterations {[}{]}}\label{hamt-surface-linearization-iterations}

This output is the number of solver iterations taken on the most recent time step for the surface. For the Thomas and BDF2 schemes it is the number of outer Picard iterations (1 indicates the fast linearized path was sufficient; 2 or more indicates the linearization safety net refreshed material properties and iterated again). For the Gauss-Seidel scheme it is the number of sweeps performed. It is a diagnostic for tuning the convergence and linearization-safety thresholds.
...
\item
  Zone,Average,HAMT Surface Vapor Pressure Cell N {[}Pa{]}
\item
  Zone,Average,HAMT Surface Heat Flux Cell N {[}W/m2{]}
\end{itemize}
...
\paragraph{HAMT Surface Vapor Pressure Cell \textless{}N\textgreater{} {[}Pa{]}}\label{hamt-surface-vapor-pressure-cell-n-pa}

This is the vapor pressure of the cell in the surface. Together with the per-cell temperature it allows the dew point to be checked at each spatial node, supporting interstitial condensation analysis through the depth of the construction.

\paragraph{HAMT Surface Heat Flux Cell \textless{}N\textgreater{} {[}W/m2{]}}\label{hamt-surface-heat-flux-cell-n-wm2}

This is the conductive heat flux at the interior-facing face of the cell in the surface, positive in the exterior-to-interior direction.
